...
const navigateToTenant = (id: string) => {
  router.push(`/tenant-admin/tenants/${id}`)
}

const navigateToOrg = (slug: string) => {
  router.push(`/tenant-admin/organizations/${slug}/overview`)
}
...
const submitDelete = async () => {
  ...
  try {
    ...
    closeDeleteModal()
    router.replace({ path: '/tenant-admin/tenants', query: { deleted: tenant.value.slug } })
  } catch (err: any) {
