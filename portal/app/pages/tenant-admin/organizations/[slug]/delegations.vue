              ...
              <summary class="cursor-pointer px-6 py-4 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                <div class="flex items-center justify-between gap-3">
                  ...
                  <div class="flex items-center gap-2 flex-shrink-0">
                    ...
                    >
                      {{ group.activeCount }} aktiv{{ group.activeCount !== 1 ? 'a' : '' }}
                    </span>
                    <span
                    ...
                    >
                      {{ group.expiredCount }} utgången{{ group.expiredCount !== 1 ? 'a' : '' }}
                    </span>
                    <span
                    ...
                    >
                      {{ group.revokedCount }} återkallad{{ group.revokedCount !== 1 ? 'a' : '' }}
                    </span>
                    <span class="text-xs text-slate-500 dark:text-slate-400">
                      ({{ group.delegations.length }} totalt)
                    </span>
                  </div>
                </div>
              </summary>
            ...
            <div>
              ...
              <div class="mt-1">
                <input
                  ...
                  placeholder="E-post eller namn (söker automatiskt)"
                />
              </div>
              <div v-if="searchingUsers" class="mt-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-500 dark:border-white/10 dark:bg-white/5 dark:text-slate-400">
                <div class="flex items-center gap-2">
              ...
              <div v-else-if="userResults.length" class="mt-2 space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm dark:border-white/10 dark:bg-white/5">
                <p class="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Träffar ({{ userResults.length }})</p>
                <button
              ...
              >
                <div class="flex items-start gap-2">
                  <Icon icon="mdi:alert" class="h-4 w-4 flex-shrink-0 mt-0.5" />
                  <div>
                    <p class="font-semibold">
                      Användaren har redan {{ activeDelegationCount }} aktiv{{ activeDelegationCount !== 1 ? 'a' : '' }} delegation{{ activeDelegationCount !== 1 ? 'er' : '' }}
                    </p>
                    <p class="mt-1 text-amber-700 dark:text-amber-300">
                      Permissions från alla aktiva delegationer sammanfogas (union). Detta kan vara avsiktligt om du vill ge olika permissions för olika ändamål eller tidsperioder.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              <div class="mt-1 mb-2">
                <input
                  ...
                  type="text"
                  class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand dark:border-white/10 dark:bg-black/20 dark:text-white"
                  placeholder="Sök moduler eller rättigheter..."
                />
              </div>
...
// Watch userQuery and trigger search automatically
watch(userQuery, (newValue) => {
  if (newValue.trim()) {
    debouncedSearch()
  } else {
    userResults.value = []
