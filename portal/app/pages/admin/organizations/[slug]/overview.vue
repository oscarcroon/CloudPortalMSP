...
const organization = computed(() => (data.value as AdminOrganizationDetail | null)?.organization)
const provider = computed(() => (data.value as AdminOrganizationDetail | null)?.provider ?? null)
const stats = computed(() => (data.value as AdminOrganizationDetail | null)?.stats ?? { memberCount: 0, activeMembers: 0, pendingInvites: 0 })
...
const form = reactive({
  ...
  billingEmail: '',
  coreId: '',
  defaultRole: roles[3] as (typeof roles)[number],
  requireSso: false
})
...
const allProviders = computed(() => (providersData.value as TenantsResponse | null)?.tenants ?? [])
