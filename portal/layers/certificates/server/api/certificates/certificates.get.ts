...
import { certCertificates } from '~~/server/database/schema'
...
export default defineEventHandler(async (event) => {
  ...
  const conditions = [eq(certCertificates.organizationId, orgId)]
  if (query.status) conditions.push(eq(certCertificates.status, query.status as any))
  if (query.search) conditions.push(like(certCertificates.primaryDomain, `%${query.search}%`))
  ...
  const rows = await db
    ...
    .from(certCertificates)
    .where(where)
    .orderBy(desc(certCertificates.expiresAt))
