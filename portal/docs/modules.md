...
Först, lägg till modul-ID:t i `portal/src/constants/modules.ts`:
...
**OBS**: Om du behöver nya permissions, lägg till dem i `portal/src/constants/rbac.ts`:
...
Skapa en mapp under `portal/src/lib/modules/`:

```
portal/src/lib/modules/my-new-module/
...
Lägg till modulen i `portal/src/lib/modules/index.ts`:
...
Skapa en sida för modulen i `portal/src/pages/`:

```vue
<!-- portal/src/pages/my-module/index.vue -->
<template>
  <section>
    <h1>Min nya modul</h1>
    <!-- Modulens innehåll -->
...
Om modulen behöver server-funktionalitet, skapa Nitro routes under `portal/server/api/`:

```typescript
// portal/server/api/my-module/index.get.ts
...
- `role_module_role_mappings`: kopplar RBAC-roller (owner/admin/operator/...) till modulroller. Basvärden kommer från `portal/app/constants/moduleRoleMappings.ts` men tabellen kan senare utökas via UI.
...
- Filen `portal/app/constants/moduleRoleMappings.ts` beskriver vilka modulroller som gäller för respektive organisationsroll (`owner`, `admin`, `operator`, m.fl.) och används för att seed:a databasen.
...
1. **Moduldefinition**: `portal/src/lib/modules/cloudflare/index.ts`
2. **Permissions**: `cloudflare:read`, `cloudflare:write` (definierade i `rbac.ts`)
3. **Route**: `/dns` (definierat i moduldefinitionen)
4. **Sida**: `portal/src/pages/dns/index.vue`
5. **API-routes**: `portal/src/server/api/dns/*` (använder `requirePermission`)
