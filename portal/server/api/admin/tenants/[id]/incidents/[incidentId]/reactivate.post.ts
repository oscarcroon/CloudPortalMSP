...
export default defineEventHandler(async (event) => {
  ...
  if (!tenant || !['distributor', 'provider'].includes(tenant.type)) {
    throw createError({ statusCode: 403, message: 'Only distributors and providers can reactivate incidents.' })
  }
  ...
  if (existingIncident.status === 'active') {
  ...
  await logTenantAction(event, 'INCIDENT_REACTIVATED', {
    incidentId,
