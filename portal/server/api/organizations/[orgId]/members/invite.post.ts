...
type InviteTransactionResult =
  ...
  | { type: 'invite'; inviteId: string; token: string; expiresAt: number; email: string; role: OrgMemberRole }
...
export default defineEventHandler(async (event) => {
  ...
  if (result.type === 'member') {
    ...
    }).from(organizationMemberships).innerJoin(users, eq(users.id, organizationMemberships.userId)).where(eq(organizationMemberships.id, result.membershipId))
  ...
  }).from(organizationInvitations).where(eq(organizationInvitations.id, result.inviteId))
  ...
  try {
    await sendInvitationEmail({
      ...
      role: result.role, to: result.email, expiresAt: result.expiresAt, token: result.token, organizationLogo: organization.logoUrl ?? null
    })
  } catch (error) {
    console.error('[invite] Failed to send invitation email', error)
    throw createError({ statusCode: 502, message: `Invitation created but email could not be sent. ${describeEmailSendError(error)}` })
  }

  await logUserAction(event, 'USER_INVITED', { targetUserEmail: result.email, organizationId: organization.id, role: result.role, inviteId: invite.id, directAdd: false })

  return { invite: { id: invite.id, email: invite.email, role: invite.role, status: invite.status, invitedAt: invite.invitedAt, expiresAt: invite.expiresAt, invitedBy: { id: auth.user.id, email: auth.user.email, fullName: auth.user.fullName } } }
})
