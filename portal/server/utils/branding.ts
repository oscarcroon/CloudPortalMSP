...
async function convertSvgToPng(svgBuffer: Buffer, maxWidth = 300): Promise<Buffer> {
...
function isSvgFile(filename: string, mimeType?: string | null): boolean {
  return (
...
export async function setBrandingMediaAsset(
  ...
  // Auto-generate PNG email logo when an SVG is uploaded as appLogoLight
  if (mediaType === 'appLogoLight' && isSvgFile(payload.filename, payload.mimeType)) {
    try {
      // Re-read the branding row to check if an email logo was explicitly uploaded
      const currentRow = await findBrandingRow(db, target)
      const currentEmailLogoUrl = currentRow?.emailLogoUrl as string | null | undefined
      // Only auto-generate if no email logo exists, or the existing one was auto-generated
      const isAutoGenerated = currentEmailLogoUrl?.includes('-email-logo-') ?? false
      if (!currentEmailLogoUrl || isAutoGenerated) {
        const pngBuffer = await convertSvgToPng(payload.data)
        const emailLogoPayload: LogoFilePayload = {
          filename: payload.filename.replace(/\.svg$/i, '.png'),
          mimeType: 'image/png',
          data: pngBuffer
        }
        const emailConfig = BRANDING_MEDIA_CONFIG.emailLogo
        const previousEmailUrl = currentEmailLogoUrl ?? null
        const emailUrl = await persistLogoFile(target, emailLogoPayload, previousEmailUrl, emailConfig.filenameSuffix)
        await writeBrandingRecord(db, target, { emailLogoUrl: emailUrl }, userId)
        console.info('[branding] Auto-generated PNG email logo from SVG:', emailUrl)
      }
    } catch (err) {
      // Don't fail the main upload if email logo generation fails
      console.warn('[branding] Failed to auto-generate PNG email logo from SVG:', err)
    }
  }
...
async function writeBrandingRecord(
  ...
  const insertPayload: typeof brandingThemes.$inferInsert = {
    ...
    loginBackgroundUrl: Object.prototype.hasOwnProperty.call(changes, 'loginBackgroundUrl')
      ...
      : null,
    emailLogoUrl: Object.prototype.hasOwnProperty.call(changes, 'emailLogoUrl')
      ? toStringOrNull(changes.emailLogoUrl)
      : null,
