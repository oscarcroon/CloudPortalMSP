...
const baseSummary = (targetType: 'global' | 'provider' | 'distributor' | 'organization', organizationId?: string | null, tenantId?: string | null) => ({
  targetType,
...
export const getOrganizationEmailProviderSummary = async (organizationId: string) => {
  const record = await fetchByTargetKey(organizationId)
  if (!record) {
    return baseSummary('organization', organizationId)
  }
  const profile = tryDecryptProfile(record)
  return toSummary(record, profile)
}
...
export const saveOrganizationEmailProvider = async (
  organizationId: string,
  payload: SaveEmailProviderInput
) => {
  await upsertProvider(organizationId, 'organization', organizationId, null, payload)
  return getOrganizationEmailProviderSummary(organizationId)
}

const getHierarchyForOrganization = async (organizationId: string): Promise<HierarchyNode[]> => {
  const db = getDb()
  const [org] = await db
    .select({
      id: organizations.id,
...
export const getOrganizationEmailProviderProfile = async (organizationId: string) => {
  const cryptoKey = ensureCryptoKey()
  const record = await fetchByTargetKey(organizationId)
  if (record) {
    return mapToProfile(record, cryptoKey)
