  ...
  <section class="space-y-8">
    <header class="space-y-1">
      ...
      <h1 class="text-2xl font-semibold text-slate-900 dark:text-white">
        {{ tenantType === 'provider' ? 'Skapa leverantör' : 'Skapa distributör' }}
      </h1>
      <p class="text-sm text-slate-600 dark:text-slate-400">
        <span v-if="tenantType === 'provider'">Leverantörer kan skapa distributörer och se allt neråt i hierarkin.</span>
        <span v-else>Distributörer kan skapa organisationer under sig.</span>
      </p>
    </header>
    ...
    <form class="space-y-6" @submit.prevent="handleSubmit">
      <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/5">
        ...
        <div class="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <div v-if="tenantType === 'distributor'">
            ...
            >
              ...
              <option v-for="provider in suppliers" :key="provider.id" :value="provider.id">
...
// Fetch providers if creating distributor
const { data: suppliersData } = await useFetch<AdminTenantSummary[]>('/api/admin/tenants', {
  query: { type: 'provider' }
})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (tenantType.value === 'distributor' && form.parentTenantId) {
      payload.parentTenantId = form.parentTenantId
    }
