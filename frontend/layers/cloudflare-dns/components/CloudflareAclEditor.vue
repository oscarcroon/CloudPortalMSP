<template>
  <section class="mod-cloudflare-dns-panel space-y-4">
    <header class="flex items-center justify-between gap-3">
      <div>
        <p class="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Åtkomst</p>
        <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-50">Zon-ACL</h3>
      </div>
      <span v-if="!canManage" class="text-xs text-slate-500 dark:text-slate-400">Read-only</span>
    </header>
        ...
        <tbody class="divide-y divide-slate-100 bg-white dark:divide-slate-800 dark:bg-slate-900">
          <tr v-for="(row, index) in localRows" :key="`${row.principalId}-${index}`" class="text-sm text-slate-700 dark:text-slate-200">
            <td class="px-4 py-3 align-top">
              <input v-model="row.principalId" class="input" :disabled="!canManage" placeholder="user-id eller org-roll" />
            </td>
            <td class="px-4 py-3 align-top">
              <select v-model="row.principalType" class="input" :disabled="!canManage">
                <option value="user">User</option>
                <option value="org-role">Org-role</option>
              </select>
            </td>
            <td class="px-4 py-3 align-top">
              <select v-model="row.role" class="input" :disabled="!canManage">
                <option value="viewer">Viewer</option>
                <option value="records-only">Records only</option>
                <option value="editor">Editor</option>
                <option value="admin">Admin</option>
              </select>
            </td>
            <td v-if="canManage" class="px-4 py-3 text-right">
              <button class="btn-danger" type="button" @click="removeRow(index)">Ta bort</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="canManage" class="flex items-center justify-between">
      <button class="btn-secondary" type="button" @click="addRow">Lägg till rad</button>
      <button
