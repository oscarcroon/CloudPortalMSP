      ...
      >
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Hantera logotyp och accentfärg för {{ activeOrganisationName }}.
            </p>
          </div>
        </div>
...
import { computed, reactive, ref, onMounted, watch } from '#imports'
...
watch(
  () => auth.state.value.data?.currentOrgId,
  () => {
    refreshBranding()
  },
  { immediate: true }
...
const activeAccentColor = computed(
  () => brandingDetails.value?.activeTheme.accentColor ?? auth.branding.value?.activeTheme.accentColor ?? DEFAULT_BRANDING_ACCENT
)
const logoSourceLabel = computed(() => formatBrandingSource(brandingDetails.value?.activeTheme.logoSource ?? auth.branding.value?.activeTheme.logoSource))
const accentSourceLabel = computed(() => formatBrandingSource(brandingDetails.value?.activeTheme.accentSource ?? auth.branding.value?.activeTheme.accentSource))
const organizationHasCustomAccent = computed(() =>
  Boolean(
    brandingDetails.value?.organizationTheme?.accentColor ||
      brandingDetails.value?.organizationTheme?.paletteKey
  )
)
const tenantBrandingLayers = computed(() => {
