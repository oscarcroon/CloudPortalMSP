...
const inlineMarkdown = (escaped) => escaped
    .replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
    .replace(/\*(.+?)\*/g, '<em>$1</em>');
const renderLines = (lines, isDark = false) => lines
    .map((line) => line.trim())
    .filter(Boolean)
    .map((line) => `<p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:${isDark ? DEFAULT_TEXT_SECONDARY : '#0f172a'};">${inlineMarkdown(escapeHtml(line))}</p>`)
    .join('');
