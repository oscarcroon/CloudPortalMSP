...
const getCredential = (tenantId: string, clientId: string, clientSecret: string) => {
...
const formatAddress = (email: string, name?: string) => (name ? `${name} <${email}>` : email)

const recipientsToString = (recipients: EmailRecipient[]) =>
  recipients.map((recipient) => formatAddress(recipient.email, recipient.name)).join(', ')
...
export const sendViaGraph = async (
  profile: EmailProviderProfile,
  recipients: EmailRecipient[],
  subject: string,
  html: string,
